...
while [ $# -gt 0 ]; do
  case "$1" in
    ...
    --target) TARGET="${2:?--target requires an exact annotated symbol name}"; shift 2 ;;
...
[ -n "$RULE" ] || {
  echo "usage: $0 [--seed SYMBOL] [--target SYMBOL] [--profile FILE] [--jobs N] [--write] EXAMPLE.dl [index.db]" >&2
  exit 2
}
[[ "$RULE" = /* ]] || RULE="$HERE/$RULE"
...
case "$(basename "$RULE")" in
  02_callgraph.dl|03_references.dl|04_hierarchy.dl|05_architecture.dl|06_impact.dl|07_metrics.dl|08_templates.dl|10_writeback.dl|11_all_paths.dl)
    [ -n "$SEED" ] || {
...
if [ "$(basename "$RULE")" = "11_all_paths.dl" ] && [ -z "$TARGET" ]; then
  echo "error: 11_all_paths.dl requires --target with an exact annotated symbol name" >&2
  exit 2
fi
...
if [ -n "$TARGET" ]; then
  ...
  sqlite3 "$DB_ABS" "DROP VIEW query_target; CREATE VIEW query_target AS SELECT name FROM symdisp WHERE name='$ESCAPED_TARGET';"
  if [ "$(sqlite3 "$DB_ABS" 'SELECT count(*) FROM query_target;')" -eq 0 ]; then
    echo "error: no exact annotated target named: $TARGET" >&2
    echo "candidates:" >&2
    sqlite3 "$DB_ABS" "SELECT '  ' || name FROM symdisp WHERE name LIKE '%$ESCAPED_TARGET%' LIMIT 10;" >&2
    exit 3
  fi
fi
