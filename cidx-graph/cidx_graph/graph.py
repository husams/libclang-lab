"""cidx_graph.graph -- read-only, ground-the-LLM API over a cidx index.db.

This module lets an agent REASON over a large code graph without loading source
files into its context. You write a small Python snippet that calls these
methods, execute it, and read back a compact result (symbols + file:line sites).
The traversal happens here, in SQLite -- not in the model's context window.

It depends only on the Python standard library (sqlite3). It never writes to the
database. The schema it reads is cidx schema v7:

    component / directory / file   location of every symbol (path is rebuilt by
                                   joining component.path / directory.path / name)
    symbol                         one decl/def, keyed by clang USR
    edge (+ edge_kind, edge_site)  typed relationships between symbols
    template_param / template_arg  generic parameters/arguments
...
Quick start:
    ...
    g = open_graph()                       # auto-discovers the cidx DB
    fn = g.find("rd_kafka_new")[0]         # fuzzy lookup
    for s in g.callers(fn):                # who calls it (incoming `calls`)
        print(s)
    for t in g.dispatch_targets(method):   # virtual method -> runtime targets
        print(t)
"""
...
# edge_kind.id <-> name (seeded identically by the indexer; we hardcode to avoid
# a query and to fail loudly if a DB ever disagrees via _check_edge_kinds()).
EDGE_KINDS = {
...
_DEFAULT_DB = "~/.cache/cidx/index.db"
_CACHE_ENV = "INDEXER_CACHE"


def default_db_path() -> str:
    """Where cidx keeps its index: $INDEXER_CACHE/index.db else ~/.cache/cidx/index.db."""
    cache = os.environ.get(_CACHE_ENV)
    if cache:
        return os.path.join(os.path.expanduser(cache), "index.db")
    return os.path.expanduser(_DEFAULT_DB)


def open_graph(db_path: Optional[str] = None) -> "Graph":
    """Open the cidx index read-only. `db_path` overrides auto-discovery."""
    return Graph(db_path or default_db_path())
...
class Sym:
    ...
    def is_stub(self) -> bool:
        """A minted placeholder for a target that was never indexed."""
...
class Edge:
    ...
    count: int                # call/use multiplicity (rolled up by `cidx resolve`)
...
class Graph:
    ...
    Traversals are bounded by `limit` and `depth` -- never unbounded -- so a query
    ...
    def __init__(self, db_path: str):
        if not os.path.exists(db_path):
            raise FileNotFoundError(
                ...
                "cidx index && cidx resolve\n"
                "or pass open_graph(db_path=...) / set $INDEXER_CACHE."
            )
    ...
    def _kind_ids(self, kinds: Optional[Iterable[str]]) -> Optional[list[int]]:
        ...
        for k in kinds:
            if k not in EDGE_KINDS:
                raise ValueError(f"unknown edge kind {k!r}; valid: {sorted(EDGE_KINDS)}")
    ...
    def _edges(self, sym, direction: str, kinds, limit: int) -> list[Edge]:
        ...
        sql = (f"SELECT e.id AS eid, e.src_id, e.dst_id, e.kind AS ekind, e.count, "
        ...
        if kids:
            ...
            args.extend(kids)
        sql += " ORDER BY e.count DESC, e.kind LIMIT ?"
        args.append(limit)
        out = []
        for r in self._c.execute(sql, args):
            out.append(Edge(
                ...
                count=r["count"], base_access=r["base_access"],
    ...
    def sql(self, query: str, params: Sequence = ()) -> list[sqlite3.Row]:
        """Run an arbitrary read-only SELECT for queries the helpers don't cover.
        See references/schema.md for the table layout. Rows are sqlite3.Row
        (index by column name)."""
        q = query.lstrip().lower()
        if not (q.startswith("select") or q.startswith("with")):
            raise ValueError("Graph.sql is read-only: use SELECT/WITH only")
        return self._c.execute(query, params).fetchall()
...
@dataclass
class Traversal:
    """Result of Graph.walk(): reached symbols + how they were reached."""
    ...
    def __repr__(self) -> str:
        return f"Traversal({len(self.nodes_by_id)} nodes, max_depth={max(self.depth_by_id.values(), default=0)})"
