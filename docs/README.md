...
| 7   | [Capstone Project: `cidx`](part_7_capstone_project.md)                  | Project brief (no shipped scripts): build a real CLI symbol indexer + cross-TU call-graph builder — `compile_commands.json` in, SQLite index out, then graph queries (transitive callers/callees, paths, cycles, dead code, DOT) — composing §4.5, §5.4, §6.7, §6.5. Architecture, schema, milestones, and a definition-of-done backlog |
