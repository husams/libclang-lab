...
def _resolve_symbol(db: Storage, args) -> tuple[Symbol | None, int]:
    ...
    if not hits:
        print(f"error: no symbol matches --name {args.name!r}"
              + (f" (kind {args.kind})" if args.kind else ""), file=sys.stderr)
        return None, 1
    if len(hits) > 1 and not args.first:
        print(f"error: --name {args.name!r} matches {len(hits)} symbols; "
...
def resolve_target(args) -> tuple[Target | None, int]:
    ...
    if target:
        if "://" in target:                              # COMPONENT://PATH
            with Storage(args.index) as db:
                ...
                if comp is None:
                    print(f"error: no component named {comp_name!r}",
                          file=sys.stderr)
                    return None, 1
                abs_path = os.path.normpath(os.path.join(comp.path, rel.lstrip("/")))
                rec = db.get_file(abs_path)
                if rec is None:
                    print(f"error: not in index database: {abs_path}",
                          file=sys.stderr)
                    return None, 1
                return Target(abspath=abs_path,
                              flags=list(rec.compile_options or []),
                              driver=rec.driver, focus_usr=focus_usr,
                              focus_name=focus_name), 0

        abs_path = os.path.abspath(os.path.expanduser(target))
        if adhoc_flags:                                  # explicit ad-hoc flags
            return Target(abspath=abs_path, flags=adhoc_flags,
                          focus_usr=focus_usr, focus_name=focus_name), 0
        ...
        if os.path.exists(abs_path):
            print(f"warning: {abs_path} is not in the index and no flags were "
                  f"given (pass '-- <flags>'); parsing with defaults",
                  file=sys.stderr)
            return Target(abspath=abs_path, flags=[], focus_usr=focus_usr,
                          focus_name=focus_name), 0
    ...
    if not _has_selector(args):
        print("error: need a symbol selector (--usr/--id/--name) or a "
              "FILE/COMPONENT://PATH target", file=sys.stderr)
        return None, 2
    with Storage(args.index) as db:
        ...
        if file_id is None:
            print(f"error: symbol {sym.spelling!r} has no indexed file "
                  f"(declaration-only/external)", file=sys.stderr)
            return None, 1
        rec = db.get_file_by_id(file_id)
        path = db.file_abs_path(file_id)
        if rec is None or path is None:
            print("error: cannot resolve the symbol's file in the index",
                  file=sys.stderr)
            return None, 1
        return Target(abspath=path, flags=list(rec.compile_options or []),
                      driver=rec.driver, focus_usr=sym.usr, symbol=sym), 0


def _parse_target(t: Target) -> cx.TranslationUnit | None:
    try:
        return parse(t.abspath, t.flags, driver=t.driver, check=False)
...
def _cursor_json(c: cx.Cursor, depth: int, max_depth: int | None,
                 want_tokens: bool, want_types: bool) -> dict:
    d: dict = {
        "kind": c.kind.name,
    ...
    if max_depth is None or depth < max_depth:
        kids = [_cursor_json(ch, depth + 1, max_depth, want_tokens, want_types)
                for ch in c.get_children()]
        if kids:
            d["children"] = kids
    return d
...
def _dump_text(c: cx.Cursor, depth: int, max_depth: int | None,
               want_tokens: bool, want_types: bool) -> None:
    indent = "  " * depth
...
def cmd_dump(args) -> int:
    ...
    if t.whole_file:
        ...
        roots = [c for c in tu.cursor.get_children()
                 ...
                 and c.location.file.name == t.abspath]
    else:
        focus = _find_focus(tu, t)
        if focus is None:
            sel = t.focus_usr or t.focus_name
            print(f"error: could not locate {sel!r} in {os.path.basename(t.abspath)}",
                  file=sys.stderr)
            return 1
        roots = [focus]
...
def _focus_function(tu: cx.TranslationUnit, t: Target) -> cx.Cursor | None:
    if t.whole_file:
        print("error: this command needs a function "
    ...
    if focus.kind not in _FUNCTION_KINDS:
        print(f"error: {focus.spelling!r} is a {focus.kind.name}, not a function",
              file=sys.stderr)
        return None
    return focus
...
def cmd_conditions(args) -> int:
    ...
    for call in calls:
        ...
        guarded = sorted({c.spelling for c in calls
                          if _guarded_by(c, guard, parent_of, focus)})
        row = {
...
def _guarded_by(call: cx.Cursor, guard: cx.Cursor,
                parent_of: dict[int, cx.Cursor], focus: cx.Cursor) -> bool:
    node = parent_of.get(call.hash)
    while node is not None and node.hash != focus.hash:
