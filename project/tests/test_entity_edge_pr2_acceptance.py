...
Scenarios covered (mapped to DESIGN_entity_edge_plan.md §PR2 test matrix):
  ...
  version-1     Python VERSION == "0.32.0"  (entity_graph reader API)
  version-2     C++ kVersion == "0.32.0"
...
def test_python_version_is_0300():
    """Python VERSION must be 0.32.0 (entity_graph reader API added)."""
    ...
    assert version == "0.32.0", (
        f"Python VERSION is '{version}'; expected '0.32.0'. "
        "Bump VERSION to 0.32.0 in cli.py (entity_graph reader API)."
    )


def test_cpp_version_is_0300():
    """C++ kVersion must be 0.32.0 (entity_graph reader API added)."""
    args_src = _read(_ARGS_HPP)
    match = re.search(r'kVersion\s*=\s*"([^"]+)"', args_src)
    assert match is not None, "kVersion not found in args.hpp."
    version = match.group(1)
    assert version == "0.32.0", (
        f"C++ kVersion is '{version}'; expected '0.32.0'. "
        "Bump kVersion to 0.32.0 in args.hpp (entity_graph reader API)."
    )
