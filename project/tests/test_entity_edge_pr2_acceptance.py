...
Scenarios covered (mapped to DESIGN_entity_edge_plan.md §PR2 test matrix):
  ...
  version-1     Python VERSION == "0.25.0"  (instantiates entity_edge kind)
  version-2     C++ kVersion == "0.25.0"
...
def test_python_version_is_024():
    """Python VERSION must be 0.25.0 after the named-instance composes feature."""
    ...
    assert version == "0.25.0", (
        f"Python VERSION is '{version}'; expected '0.25.0'. "
        "Bump VERSION to 0.25.0 in cli.py (named-instance composes feature)."
    )


def test_cpp_version_is_024():
    """C++ kVersion must be 0.25.0 after the named-instance composes feature."""
    args_src = _read(_ARGS_HPP)
    match = re.search(r'kVersion\s*=\s*"([^"]+)"', args_src)
    assert match is not None, "kVersion not found in args.hpp."
    version = match.group(1)
    assert version == "0.25.0", (
        f"C++ kVersion is '{version}'; expected '0.25.0'. "
        "Bump kVersion to 0.25.0 in args.hpp (named-instance composes feature)."
    )
