...
Scenarios covered (mapped to DESIGN_entity_edge_plan.md §PR2 test matrix):
  ...
  version-1     Python VERSION == "0.28.1"  (instantiates entity_edge kind)
  version-2     C++ kVersion == "0.28.1"
...
def test_python_version_is_0281():
    """Python VERSION must be 0.28.1 after the owning-record composes feature."""
    ...
    assert version == "0.28.1", (
        f"Python VERSION is '{version}'; expected '0.28.1'. "
        "Bump VERSION to 0.28.1 in cli.py (owning-record composes named instance)."
    )


def test_cpp_version_is_0281():
    """C++ kVersion must be 0.28.1 after the owning-record composes feature."""
    args_src = _read(_ARGS_HPP)
    match = re.search(r'kVersion\s*=\s*"([^"]+)"', args_src)
    assert match is not None, "kVersion not found in args.hpp."
    version = match.group(1)
    assert version == "0.28.1", (
        f"C++ kVersion is '{version}'; expected '0.28.1'. "
        "Bump kVersion to 0.28.1 in args.hpp (owning-record composes named instance)."
    )
