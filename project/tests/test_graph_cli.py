...
def test_callers_json(resolved_db, capsys):
    rc, out, _ = run(
        ["graph", "callers", "--db", resolved_db, "--name", "helper", "--json"],
        capsys)
    assert rc == 0
    data = json.loads(out)
    ...
    for key in ("id", "usr", "qual_name", "kind", "file", "line", "count",
                "edge_kind", "sites"):
        assert key in d
...
def test_refs_json(resolved_db, capsys):
    rc, out, _ = run(
        ["graph", "refs", "--db", resolved_db, "--name", "g_config", "--json"],
        capsys)
    assert rc == 0
    data = json.loads(out)
...
def test_selector_by_usr(resolved_db, capsys):
    rc, out, _ = run(
        ["graph", "callers", "--db", resolved_db, "--usr", "c:@F@helper",
         "--json"], capsys)
    assert rc == 0
...
def test_selector_by_id(resolved_db, capsys):
    ...
    rc, out, _ = run(
        ["graph", "callers", "--db", resolved_db, "--id", str(helper_id),
         "--json"], capsys)
    assert rc == 0
    assert [d["qual_name"] for d in json.loads(out)] == ["main"]
...
def test_ambiguous_name_first_wins(resolved_db, capsys):
    rc, out, err = run(
        ["graph", "dispatch", "--db", resolved_db, "--name", "draw", "--first",
         "--json"], capsys)
    assert rc == 0
...
def test_neighbors_edge_filter(resolved_db, capsys):
    rc, out, _ = run(
        ["graph", "neighbors", "--db", resolved_db, "--usr", "c:@S@Base",
         "--edge", "inherits", "--direction", "in", "--json"], capsys)
    assert rc == 0
...
def test_walk_depth_bound(resolved_db, capsys):
    rc, out, _ = run(
        ["graph", "walk", "--db", resolved_db, "--name", "main",
         "--edge", "calls", "--depth", "1", "--json"], capsys)
    assert rc == 0
    data = json.loads(out)
    assert {d["qual_name"] for d in data} == {"helper"}
...
def test_path_found(resolved_db, capsys):
    rc, out, _ = run(
        ["graph", "path", "--db", resolved_db, "--usr", "c:@F@main",
         "--to-usr", "c:@F@compute", "--json"], capsys)
    assert rc == 0
    assert [d["qual_name"] for d in json.loads(out)] == ["main", "helper",
                                                         "compute"]


def test_path_not_found(resolved_db, capsys):
    rc, out, _ = run(
        ["graph", "path", "--db", resolved_db, "--usr", "c:@F@compute",
         "--to-usr", "c:@F@main", "--json"], capsys)
    assert rc == 1
...
def test_hierarchy_json(resolved_db, capsys):
    rc, out, _ = run(
        ["graph", "hierarchy", "--db", resolved_db, "--usr", "c:@S@Derived2",
         "--transitive", "--json"], capsys)
    assert rc == 0
    data = json.loads(out)
...
def test_hierarchy_members_union(resolved_db, capsys):
    rc, out, _ = run(
        ["graph", "hierarchy", "--db", resolved_db, "--usr", "c:@S@Base",
         "--json"], capsys)
    assert rc == 0
    data = json.loads(out)
...
def test_dispatch_pure_root(resolved_db, capsys):
    rc, out, _ = run(
        ["graph", "dispatch", "--db", resolved_db, "--usr",
         "c:@S@Base@F@draw#", "--json"], capsys)
    assert rc == 0
    data = json.loads(out)
    assert data["is_virtual"] is True
    assert {t["qual_name"] for t in data["targets"]} == {
        "Derived::draw", "Derived2::draw"}
...
def test_missing_db_errors(tmp_path, capsys):
    ...
    rc, out, err = run(
        ["graph", "callers", "--db", missing, "--name", "x"], capsys)
    assert rc == 1
...
def test_edgeless_db_errors_no_substitution(empty_db, capsys):
    rc, out, err = run(
        ["graph", "callers", "--db", empty_db, "--name", "main"], capsys)
    assert rc == 1
    assert "no graph edges" in err
    assert out == ""


def test_default_db_used_when_no_db_flag(resolved_db, monkeypatch, tmp_path,
                                         capsys):
    ...
    rc, out, _ = run(
        ["graph", "callers", "--name", "helper", "--json"], capsys)
    assert rc == 0
    assert [d["qual_name"] for d in json.loads(out)] == ["main"]
